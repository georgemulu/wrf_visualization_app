...
<div class="gradient-header">
    <h1 style="margin:0; color:white; text-align:center;">🌦️ Weather Analytics Dashboard</h1>
</div>
""", unsafe_allow_html=True)
...
if nc:
    ...
    with st.expander("⚙️ CONTROL PANEL", expanded=True):
        ...
        with col3:
            ...
            pressure_level = st.selectbox("↕️ Pressure Level", pressure_levels) if var_type == 'pressure' else None
    ...
    icon = next((v for k, v in icon_map.items() if k in selected_var_name), '🌤️')
    st.markdown(f"""
    <script>
        document.getElementById('icon1').innerHTML = '{icon}';
        document.getElementById('icon2').innerHTML = '{'📈' if var_type == 'surface' else '↕️'}';
    </script>
    """, unsafe_allow_html=True)
    ...
    if 'Temperature' in selected_var_name and (pressure_level == 850 or var_type == 'surface'):
        ...
        with anomaly_cols[0]:
            st.metric("🌡️ Climatology", f"{climatology:.1f}°C")
        with anomaly_cols[1]:
            st.metric("🌡️ Current Mean", f"{np.mean(current_data):.1f}°C", 
        ...
        if abs(z_score) > 2.58:
            ...
            st.warning("⚠️ Significant anomaly detected (p < 0.05)")
        else:
